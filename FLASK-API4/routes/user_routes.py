...
user_args.add_argument('username', type=str, required=True, help='Nome não pode ser vazio') # Adiciona um argumento 'name' do tipo string, obrigatório e com mensagem de erro se estiver em branco
...
user_args.add_argument('password', type=str, required=True, help='Senha não pode ser vazia') # Adiciona um argumento 'password' do tipo string, obrigatório e com mensagem de erro se estiver em branco


userFields = { # Define os campos que serão retornados na resposta da API
    'id': fields.Integer, # Campo 'id' do tipo inteiro
    'username': fields.String, # Campo 'name' do tipo string
...
class Users(Resource): # Cria uma classe 'Users' que herda de 'Resource' (do Flask-RESTful)
    ...
    def post(self): # Método POST para criar um novo usuário
        ...
        user = UserModel(username=args['username'], email=args['email'], password=args['password']) # Cria um novo usuário com os dados fornecidos
        db.session.add(user)
...
class User(Resource): # Cria uma classe 'User' que herda de 'Resource'
    ...
    def patch(self, id): # Método PATCH para atualizar um usuário específico
        ...
        if not user:
            abort(404, message="Usuário não encontrado.") # Se o usuário não for encontrado, retorna um erro 404
        user.username = args['username'] # Atualiza o nome do usuário

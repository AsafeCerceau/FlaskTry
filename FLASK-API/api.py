...
# define uma tabela chamda 'user_model' no banco. Herda de 'db.Model', que transforma a classe num modelo de banco de dados.
class UserModel(db.Model):
    # define a colna "id" como tipo inteiro, com chave primária
    id = db.Column(db.Integer, primary_key=True)
    # define a coluna 'name' como tipo string, com no máximo 80 caracteres // 'unique=True': não pode haver 2 usuários iguais // 'nullable=False': não pode ser nulo
    name = db.Column(db.String(80), unique=True, nullable=False)
    # define a coluna 'email' como tipo string, com no máximo 80 caracteres
    email = db.Column(db.String(80), unique=True, nullable=False)

    # Método especial que define como o objeto será impresso (útil para debug e logs).
    def __repr__(self):
